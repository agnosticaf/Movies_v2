...
export class MovieComponent implements OnInit, AfterViewChecked {
    ...
    this.socket.on('delete-yes', function (data) {
      ...
      if (this.current !== 'main') {
        videos = this.myVideos;
          for (const key in videos) {
          if (videos[key]._id === data)
            this.myVideos.splice(key, 1);
          }
      } else {
        videos = this.allVideos;
        for (const key in videos) {
          if (videos[key]._id === data)
            this.allVideos.splice(key, 1);
          }
      }
      console.log(data);
    }.bind(this));
  ...
  login() {
    if (
      ...
      var userData = { username: this.newUser.username, password: this.newUser.password };
      this.movieService.loginUser(userData).then((res) => {
        console.log(res)
  ...
  upload() {
    ...
    this.movieService.videoUpload(this.video).then((res) => {
      console.log(res);
      this.profile();
    }, error => {
